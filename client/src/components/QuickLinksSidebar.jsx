        ...
        <div className="flex flex-col gap-4 mr-14 bg-transparent">
          {links.map((link, idx) => (
            <div key={idx} className="flex items-center justify-end">
              <div className="mr-3 px-5 py-2 bg-white text-[rgb(115,25,25)] text-[11px] font-bold rounded-full shadow-md uppercase tracking-wider whitespace-nowrap">
                {link.label}
              </div>
              ...
              {link.to ? (
                <Link
                  to={link.to}
                  className="w-11 h-11 bg-gradient-to-br from-[rgb(115,63,63)] to-[rgb(115,25,25)] text-white flex items-center justify-center rounded-full shadow-lg hover:scale-110 transition"
                >
                  <svg
                    className="w-5 h-5"
                    fill="none"
