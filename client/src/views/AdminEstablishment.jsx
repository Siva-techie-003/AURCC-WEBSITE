...
const AdminEstablishment = () => {
    ...
    useEffect(() => {
        fetch("/api/administration")
            .then(res => res.json())
            .then(data => {
                setSection(data?.general_administration_i?.ESTABLISHMENT_SECTION);
                setLoading(false);
            })
    ...
    if (!section) return <p className="text-center mt-20">No staff data found</p>;
            ...
            <div className="mx-auto py-4 sm:py-6 lg:py-8 px-2 md:px-4 font-serif">
                <div className="max-w-7xl mx-auto flex flex-col gap-6 sm:gap-8">
                    ...
                    <div className="text-center">
                        <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-800 font-serif uppercase mb-2">General Administration-I</h2>
                        <div className="flex justify-center mb-4">
                            <span className="block w-24 h-1.5 rounded-full bg-yellow-500"></span>
                        </div>
                        <h3 className="text-base sm:text-lg lg:text-xl font-semibold text-gray-600 font-serif uppercase tracking-wide">Establishment Section</h3>
                    </div>
